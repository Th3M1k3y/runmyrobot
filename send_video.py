...
def startVideoCaptureLinux():
    ...
    if (args.brightness is not None) or (args.contrast is not None) or (args.saturation is not None):
        ...
        os.system("v4l2-ctl -c brightness={brightness} -c contrast={contrast} -c saturation={saturation}".format(brightness=args.brightness, contrast=args.contrast, saturation=args.saturation))
    else:
        print "camera settings at default"

    overlayCommand = '-vf dynoverlay=overlayfile=/dev/shm/wifi.png:x=10:y=413:check_interval=500,dynoverlay=overlayfile=/dev/shm/battery.png:x=97:y=413:check_interval=500'
    videoCommandLine = '/usr/local/bin/ffmpeg -f v4l2 -framerate 25 -video_size 640x480 -i /dev/video%s %s -f mpegts -codec:v mpeg1video -s 640x480 -b:v %dk -bf 0 -muxdelay 0.001 %s http://%s:%s/hello/640/480/' % (args.video_device_number, rotationOption(), args.kbps, overlayCommand, server, videoPort)
...
def main():
    ...
    while True:
        ...
        socketIO.emit('send_video_status', {'send_video_process_exists': True, 'ffmpeg_process_exists': True, 'camera_id':args.camera_id})
